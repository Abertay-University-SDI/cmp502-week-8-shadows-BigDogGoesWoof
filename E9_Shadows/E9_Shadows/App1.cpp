...
void App1::finalPass()
{
	...
	shadowShader->setShaderParameters(renderer->getDeviceContext(), worldMatrix, viewMatrix, projectionMatrix, textureMgr->getTexture(L"brick"), depthArray, light, lightRed); //Light Arrays may not work, may need to be made after light objects are made
	...
	shadowShader->setShaderParameters(renderer->getDeviceContext(), worldMatrix, viewMatrix, projectionMatrix, textureMgr->getTexture(L"brick"), depthArray1, light, lightRed);
	shadowShader->render(renderer->getDeviceContext(), model->getIndexCount());
